...
function extractNameFromTemplate(value) {
  const res = value.slice(7, value.length - 1);
  return res;
}
...
function encodeToRot13(str) {
  ...
  str.split('').forEach((i) => {
    ...
    if (!charArr.includes(lowerI)) {
      res += i;
    } else if (i === i.toUpperCase()) {
      const dex = charArr.indexOf(lowerI);
      res += charArr[(dex + 13) % 26].toUpperCase();
    } else {
      const dex = charArr.indexOf(i);
      res += charArr[(dex + 13) % 26];
    }
  });
  return res;
}
